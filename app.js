...
const state = {
    ...
    tramosPos: [],             // Tramos de posesión cerrados: { buttonId, equipo, start, end }. Estadística, no eventos: no van al registro ni al XML
    etiquetasFijas: [],        // Ids de las etiquetas fijas encendidas: se pegan solas a todo lo que se marca
    logAbierto: false,         // El historial en vivo arranca cerrado: se abre con el botón Historial
...
const el = {
    ...
    propExclusiveSection:  D('propExclusiveSection'),
    propExclusiveLabel:    D('propExclusiveLabel'),
    propExclusiveHelp:     D('propExclusiveHelp'),
    propFijaSection:       D('propFijaSection'),
    ...
    btnExportPos:   D('btnExportPos'),
    btnToggleLog:     D('btnToggleLog'),
    btnHideLivePanel: D('btnHideLivePanel'),
    ...
    propImagenSection:   D('propImagenSection'),
    btnCambiarImagen:    D('btnCambiarImagen'),
    propAjusteImagen:    D('propAjusteImagen'),
    propOpacidadImagen:  D('propOpacidadImagen'),
...
function setMode(mode) {
    ...
    if (mode === 'setup') {
        ...
        el.inspectorPanel.classList.add('hidden');
        // El registro tapaba la botonera todo el partido: ahora se abre a pedido.
        cerrarLivePanel();
...
function bindEvents() {
    ...
    on(el.btnToggleLog, 'click', alternarLivePanel);
    on(el.btnHideLivePanel, 'click', cerrarLivePanel);
    ...
    on(el.btnClearEvents, 'click', async () => {
        if (await customConfirm('¿Borrar todos los eventos y el tiempo acumulado?', 'Limpiar Eventos', true, 'Borrar')) {
            ...
            state.posesion = {}; state.tramosPos = [];
    ...
    on(el.btnExportPos, 'click', exportPosesionCSV);
    ...
    on(el.propAjusteImagen, 'change', updateSelected);
...
// ─────────────────────────────────────────────
// ── Imágenes ──
// Abre el selector de fotos. El input.click() tiene que pasar dentro del
// toque: iOS no abre el selector si se llama más tarde.
function elegirImagen() {
    return new Promise(resolve => {
        ...
        input.addEventListener('change', () => {
            ...
            lector.onload  = () => resolve({ dataUrl: lector.result, tipo: archivo.type || '' });
...
// Las botoneras viven en el localStorage, que en el iPad ronda 5 MB para
// todo. Una foto sin procesar lo llena de un golpe, así que se achica y se
// comprime hasta ~300 KB: en el lienzo no se nota. Si trae transparencia
// (un escudo en PNG) queda en PNG para no perderla.
function comprimirImagen(dataUrl, conTransparencia) {
    return new Promise((resolve, reject) => {
        const img = new Image();
        img.onload = () => {
            let lado = 1400, calidad = 0.85;
            const probar = () => {
                ...
                if (!conTransparencia) { ctx.fillStyle = '#fff'; ctx.fillRect(0, 0, c.width, c.height); }
                ctx.drawImage(img, 0, 0, c.width, c.height);
                return conTransparencia ? c.toDataURL('image/png') : c.toDataURL('image/jpeg', calidad);
            };
            let src = probar();
            while (src.length > 400000 && lado > 300) {
                lado = Math.round(lado * 0.75);
                calidad = Math.max(0.6, calidad - 0.08);
                src = probar();
            }
            resolve({ src: src, ancho: img.naturalWidth, alto: img.naturalHeight });
        };
...
// ¿Lo guardado entró de verdad en la memoria del dispositivo? Si no entra,
// lsSet lo deja solo en memoria y se perdería al cerrar la app sin avisar.
function entroEnMemoria() {
    try { return localStorage.getItem('tv_elements') === JSON.stringify(state.elements); }
    catch (err) { return true; }    // sin localStorage (archivo local): nada que comparar
}

const AVISO_SIN_LUGAR = 'La imagen no entra en la memoria del dispositivo: se vería ahora pero se perdería al cerrar la app. ' +
                        'Probá con una imagen más chica, o borrá otras imágenes o plantillas que no uses.';

async function insertarImagen(existente) {
    const archivo = await elegirImagen();
    if (!archivo) return;
    let img;
    try {
        img = await comprimirImagen(archivo.dataUrl, /png|gif|webp|svg/i.test(archivo.tipo));
    } catch (err) {
        customAlert('No se pudo abrir esa imagen.', 'Imagen');
        return;
    }

    if (existente) {
        const anterior = existente.src;
        existente.src = img.src;
        saveData();
        if (!entroEnMemoria()) {
            existente.src = anterior;
            saveData();
            customAlert(AVISO_SIN_LUGAR, 'Imagen');
        }
        renderAll();
        return;
    }

    // Nace con la proporción de la foto, hasta 360 × 300 px.
    const k = Math.min(1, 360 / img.ancho, 300 / img.alto);
    createElement('image', {
        src: img.src,
        w: Math.max(40, snap(img.ancho * k)),
        h: Math.max(40, snap(img.alto * k)),
        ajuste: 'contain',
        opacidad: 100
    });
    if (!entroEnMemoria()) {
        const nueva = state.selectedId;
        state.elements = state.elements.filter(e => e.id !== nueva);
        selectElement(null);
        saveData();
        renderAll();
        customAlert(AVISO_SIN_LUGAR, 'Imagen');
...
function createElement(type, extra) {
    // Insertar ▼ → Imagen primero pide la foto; con la foto lista vuelve acá.
    if (type === 'image' && !extra) { insertarImagen(); return; }
    ...
    if (extra) Object.assign(newEl, extra);
...
function mostrarInspectorMultiple() {
    ...
    if (el.propMultiTransparencia) {
        const aplicables = seleccion.filter(e => e.type !== 'image');
        const valores = new Set(aplicables.map(e => String(parseInt(e.transparencia) || 100)));
        const s = el.propMultiTransparencia;
        const previa = s.querySelector('option[value="*"]');
        if (previa) previa.remove();
        if (valores.size > 1) {
            const o = document.createElement('option');
            o.value = '*';
            o.textContent = 'Distintas: elegí una para todos';
            s.insertBefore(o, s.firstChild);
        }
        s.value = valores.size > 1 ? '*' : ([...valores][0] || '100');
        s.disabled = aplicables.length === 0;
    }
}
...
function aplicarTransparenciaMultiple() {
    const v = el.propMultiTransparencia.value;
    if (v === '*') return;
    state.elements.forEach(e => {
        if (state.selectedIds.includes(e.id) && e.type !== 'image') e.transparencia = parseInt(v) || 100;
    });
    saveData();
    renderElements();
    mostrarInspectorMultiple();
...
function selectElement(id) {
    ...
    if (el.propTransparencia) el.propTransparencia.value = String(parseInt(e.transparencia) || 100);
    if (e.type === 'image' && el.propAjusteImagen) {
        el.propAjusteImagen.value   = e.ajuste === 'cover' ? 'cover' : 'contain';
        el.propOpacidadImagen.value = String(parseInt(e.opacidad) || 100);
    }
...
function openExclusiveModal() {
    ...
    if (!current || (current.type !== 'event' && current.type !== 'sticky_label')) return;
    // Un evento se cruza con eventos, y una etiqueta automática con etiquetas.
    const esFija = current.type === 'sticky_label';

    if (!current.exclusiveIds) current.exclusiveIds = [];

    el.exclusiveModalTitle.textContent = esFija
        ? `Se apagan al encender "${current.name}"`
        : `Excluyentes con "${current.name}"`;
    el.exclusiveEventsList.innerHTML = '';

    // Solo de la misma pestaña: excluir contra un botón de otra no tiene sentido.
    const otherEvents = state.elements.filter(item => item.type === current.type && item.id !== current.id
                                                   && hojaDe(item) === hojaDe(current));

    if (otherEvents.length === 0) {
        el.exclusiveEventsList.innerHTML = `<div class="p-4 text-center text-sm text-gray-500">${esFija ? 'No hay otras etiquetas automáticas en la plantilla.' : 'No hay otros eventos en la plantilla.'}</div>`;
    } else {
...
function syncSections(type) {
    ...
    // Los excluyentes son de los eventos y de las etiquetas automáticas: en
    // unos cortan el turno abierto, en otras apagan la etiqueta encendida.
    if (el.propExclusiveSection) {
        const conExcluyentes = (type === 'event' || type === 'sticky_label');
        el.propExclusiveSection.style.display = conExcluyentes ? 'block' : 'none';
        if (el.propExclusiveLabel) {
            el.propExclusiveLabel.textContent = (type === 'sticky_label')
                ? 'ETIQUETAS QUE SE APAGAN' : 'EVENTOS EXCLUYENTES';
        }
        if (el.propExclusiveHelp) {
            el.propExclusiveHelp.textContent = (type === 'sticky_label')
                ? 'Al encender esta etiqueta, se apagan las seleccionadas.'
                : 'Al activar este evento, se cortarán los eventos seleccionados.';
        }
    }
    if (el.propFijaSection) el.propFijaSection.style.display = (type === 'sticky_label') ? 'block' : 'none';
    ...
    if (el.propImagenSection)   el.propImagenSection.style.display   = (type === 'image')      ? 'block' : 'none';
    if (el.propTransparenciaSection) el.propTransparenciaSection.style.display = (type === 'image') ? 'none' : 'block';
...
function updateSelected() {
    ...
    if (e.type !== 'image' && el.propTransparencia) {
        e.transparencia = parseInt(el.propTransparencia.value) || 100;
    }
    if (e.type === 'image' && el.propAjusteImagen) {
        e.ajuste   = el.propAjusteImagen.value === 'cover' ? 'cover' : 'contain';
        e.opacidad = parseInt(el.propOpacidadImagen.value) || 100;
    }
...
function escalaParaEntrar() {
    ...
    return { escala: escala, ancho: ancho, alto: alto, derecha: derecha, abajo: abajo };
}

function aplicarEscalaLienzo() {
    const medida = escalaParaEntrar();
    if (medida === null) return;
    const necesaria = medida.escala;
    // Codificando se ajusta siempre: la pantalla queda quieta, sin scroll, y
    // cada botón siempre en el mismo lugar. El "Tamaño real" es del editor.
    escalaLienzo = (state.mode === 'live' || ajustarActivo()) ? necesaria : 1;
    ...
    // Lienzo quieto mientras codificás: sin scroll ni rebote, salvo que la
    // botonera sea tan grande que ni achicada al máximo entre en la pantalla
    // (ahí sin scroll quedarían botones afuera, imposibles de tocar).
    if (el.canvasContainer) {
        const entra = medida.derecha * escalaLienzo <= medida.ancho + 1
                   && medida.abajo   * escalaLienzo <= medida.alto  + 1;
        const quieto = state.mode === 'live' && entra;
        el.canvasContainer.classList.toggle('sin-scroll', quieto);
        if (quieto) { el.canvasContainer.scrollTop = 0; el.canvasContainer.scrollLeft = 0; }
    }
...
function renderElements() {
    ...
    // Contenedores e imágenes se dibujan primero: van detrás de los botones,
    // así una cancha de fondo no tapa los eventos.
    const alFondo = e => e.type === 'container' || e.type === 'image';
    const containers = fuente.filter(alFondo);
    const others     = fuente.filter(e => !alFondo(e));
    ...
    [...containers, ...visible].forEach((e) => {
        ...
        if (state.mode === 'live' && e.type === 'sticky_label' && (state.etiquetasFijas || []).includes(e.id)) div.classList.add('fija-activa');
        div.style.left   = e.x + 'px';
        div.style.top    = e.y + 'px';
        div.style.width  = e.w + 'px';
        div.style.height = e.h + 'px';
        div.style.zIndex = (isContainer || e.type === 'image') ? 1 : 10;
        // Transparencia del botón, para ver lo que hay detrás (una cancha de
        // fondo). La imagen tiene su propia opacidad: no se suman las dos.
        ...
        if (isContainer) {
            ...
            div.innerHTML = e.src
                ? `<img class="img-libre" src="${e.src}" alt="" draggable="false" ` +
                  `style="object-fit:${e.ajuste === 'cover' ? 'cover' : 'contain'};opacity:${(parseInt(e.opacidad) || 100) / 100}">`
        ...
        if (state.mode === 'live' && !isContainer && e.type !== 'text' && e.type !== 'teams' && e.type !== 'image') {
            if (enDetalle) {
...
function makeEventInstance(e) {
    ...
    return {
        ...
        line: null,
        descriptors: etiquetasFijasActivas()
    };
}

// Las etiquetas fijas encendidas, listas para pegarse solas a un evento nuevo.
// Van en el orden en que las encendiste, sin repetir nombres.
function etiquetasFijasActivas() {
    const nombres = [];
    (state.etiquetasFijas || []).forEach(id => {
        const fija = state.elements.find(x => x.id === id);
        if (fija && !nombres.includes(fija.name)) nombres.push(fija.name);
    });
    return nombres;
}

// Tocar una etiqueta fija la enciende; tocarla otra vez la apaga. Se pueden
// tener varias encendidas a la vez (período + zona + rival, por ejemplo) y
// todas se pegan solas a cada evento que marques. No arranca el reloj: se
// pueden dejar puestas antes del PLAY.
function tocarEtiquetaFija(e) {
    const activas = state.etiquetasFijas || (state.etiquetasFijas = []);
    const i = activas.indexOf(e.id);
    if (i >= 0) activas.splice(i, 1);
    else        activas.push(e.id);
    // Etiquetas del mismo grupo excluyente: encender una apaga a las otras,
    // así "1er tiempo" y "2do tiempo" no quedan las dos puestas.
    if (i < 0 && (e.exclusiveIds || []).length > 0) {
        state.etiquetasFijas = activas.filter(id => id === e.id || !e.exclusiveIds.includes(id));
    }
    renderElements();
}
...
// ─────────────────────────────────────────────
// ─────────────────────────────────────────────
// PANEL DE HISTORIAL (Registro / En hielo / Posesión)
// Mientras codificás molesta tener la lista al lado comiéndose el ancho de la
// botonera, así que arranca cerrado y se abre con el botón Historial.
// ─────────────────────────────────────────────
function abrirLivePanel() {
    state.logAbierto = true;
    if (el.livePanel) el.livePanel.classList.remove('hidden');
    actualizarBotonLog();
    renderLivePanel();
}

function cerrarLivePanel() {
    state.logAbierto = false;
    if (el.livePanel) el.livePanel.classList.add('hidden');
    actualizarBotonLog();
}

function alternarLivePanel() {
    if (state.logAbierto) cerrarLivePanel();
    else abrirLivePanel();
}

// El botón lleva la cuenta de lo marcado: con el panel cerrado sigue siendo la
// señal de que el evento entró.
function actualizarBotonLog() {
    if (!el.btnToggleLog) return;
    const n = (state.events || []).length;
    el.btnToggleLog.textContent = n > 0 ? `Historial (${n})` : 'Historial';
    el.btnToggleLog.classList.toggle('log-abierto', !!state.logAbierto);
}
...
function renderLivePanel() {
    actualizarBotonLog();
    // Cerrado no hay nada que dibujar: al abrirlo se dibuja de una.
    if (!state.logAbierto) return;
...
// cuenta para ninguno.
// Los tramos NO son eventos: no ensucian el registro ni salen como clips en el
// XML. Quedan aparte, en state.tramosPos, y solo alimentan la estadística del
// panel Posesión (y su CSV). Se guardan con la sesión para poder revisarla.
// ─────────────────────────────────────────────
...
function cerrarTramoPosesion(e) {
    ...
    if (!state.tramosPos) state.tramosPos = [];
    state.tramosPos.push({
        buttonId: e.id,
...
// `tramos` son los tocados en el botón de Posesión (estadística pura); si no
// se pasan, los de la codificación que está a la vista.
function tramosDePosesion(eventos, ahora, conAbiertos, tramos) {
    const marcas = [];
    eventos.forEach(ev => {
        // posesionDe: clips de posesión de codificaciones viejas, de cuando
        // cada tramo era un evento más. Siguen contando igual.
    ...
    (tramos === undefined ? (state.tramosPos || []) : (tramos || [])).forEach(t => {
...
// Los números de posesión listos para guardar o exportar.
function resumenPosesion() {
    const t = tiemposPosesion();
    return { A: t.A, B: t.B, pctA: t.pctA, pctB: t.pctB, tramos: t.tramos };
}
...
function exportPosesionCSV() {
    const t = tiemposPosesion();
    if (!t.tramos) { customAlert('Todavía no hay posesión acumulada.', 'Exportar posesión'); return; }
    const nombres = nombresEquipos();
    let csv = 'Equipo,Posesión (%),Tiempo (s),Tiempo (mm:ss)\n';
    ['A', 'B'].forEach(eq => {
        csv += `"${String(nombres[eq]).replace(/"/g, '""')}",${t['pct' + eq] === null ? '' : t['pct' + eq]},${t[eq].toFixed(1)},${fmt(t[eq])}\n`;
    });
    saveBlobToFiles(`Posesion_${new Date().toISOString().slice(0,10)}.csv`,
                    new Blob([csv], { type: 'text/csv' }));
}
...
function armarSesion(nombre) {
    return {
        ...
        toi: toiCalculado(),
        // La posesión viaja como tramos (para recalcular al abrirla) y como
        // números ya hechos, para leerla sin la botonera que la generó.
        tramosPos: JSON.parse(JSON.stringify(state.tramosPos || [])),
        posesion: resumenPosesion()
    };
}

// Los tramos de posesión de una sesión guardada. Las viejas los traen como
// eventos (posesionDe): se convierten al vuelo.
function tramosDeSesion(s) {
    if (s && s.tramosPos && s.tramosPos.length) return JSON.parse(JSON.stringify(s.tramosPos));
    return (s && s.events || []).filter(ev => ev.posesionDe).map(ev => ({
        buttonId: ev.posesionDe, equipo: ev.equipo, start: ev.start, end: ev.end
    }));
}
...
async function saveCurrentSession() {
    if (!state.events.length && !(state.tramosPos || []).length) {
...
function exportCustomXML(eventsList, title, inicio) {
    ...
    // La posesión es estadística, no clips: va en el panel Posesión y en su
    // CSV, no en el XML. Una codificación vieja puede traer sus tramos como
    // eventos: se dejan afuera para que el archivo no los repita.
    eventsList = eventsList.filter(ev => !ev.posesionDe);
    if (!eventsList.length) { customAlert('No hay eventos para exportar.', 'Exportar XML'); return; }
