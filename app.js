...
async function saveToFiles(filename, text, mime) {
...
function exportToiCSV() {
    ...
    a.download = `ToI_${new Date().toISOString().slice(0,10)}.csv`;
    a.click();
}
...
function exportXML() {
    exportCustomXML(state.events, `Tagging_${new Date().toISOString().slice(0,10)}`);
}
...
function renderSessionsList() {
    ...
    sessions.forEach(s => {
        ...
        row.querySelector('.btn-export-sess').addEventListener('click', () => {
            exportCustomXML(s.events, s.name);
        });
...
function exportCustomXML(eventsList, title) {
    if (!eventsList || !eventsList.length) { customAlert('No hay eventos para exportar.', 'Exportar XML'); return; }
    const sorted = [...eventsList].sort((a,b)=>a.start-b.start);
    let xml = `<?xml version="1.0" encoding="UTF-8"?>\n<file>\n  <ALL_INSTANCES>\n`;
    sorted.forEach((ev,i) => {
        const startVal = typeof ev.start === 'number' ? parseFloat(ev.start.toFixed(2)) : ev.start;
        const endVal = (ev.end !== null && ev.end !== undefined) ? (typeof ev.end === 'number' ? parseFloat(ev.end.toFixed(2)) : ev.end) : startVal + 1;
        xml += `    <instance>\n      <ID>${i+1}</ID>\n      <start>${startVal}</start>\n      <end>${endVal}</end>\n      <code>${xmlEsc(ev.name)}</code>\n`;
        // Una instancia por turno; la línea que lo abrió viaja como etiqueta
        if (ev.line) {
            xml += `      <label>\n        <group>Línea</group>\n        <text>${xmlEsc(ev.line)}</text>\n      </label>\n`;
        }
        (ev.descriptors || []).forEach(d => {
            xml += `      <label>\n        <group>Etiqueta</group>\n        <text>${xmlEsc(d)}</text>\n      </label>\n`;
        });
        xml += `    </instance>\n`;
    });
    xml += `  </ALL_INSTANCES>\n</file>`;
    ...
    a.download = `${title.replace(/[^a-z0-9_\-]/gi, '_')}.xml`;
    a.click();
}
